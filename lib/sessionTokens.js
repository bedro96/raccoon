 ...
 *
 * IMPORTANT — deployment constraint: _tokens and _ipLastSubmit are in-process
 * Maps.  This is safe only when the service runs as a single replica
 * (maxReplicas: 1 in Container Apps config).  Bumping replicas without
 * introducing shared state (e.g. Redis) will break single-use token
 * enforcement and allow rate-limit bypass.
 */
