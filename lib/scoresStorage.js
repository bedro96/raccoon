...
export async function getStorageClient() {
  ...
  if (connectionString) {
    // Lazy-import so the Azure SDK is not required in non-Azure environments.
    // Using a variable prevents static analysis from failing when the package
    // is not installed locally (e.g., during tests).
    const pkg = '@azure/data-tables';
    // eslint-disable-next-line no-new-func
    const { TableClient } = await new Function('p', 'return import(p)')(pkg);
